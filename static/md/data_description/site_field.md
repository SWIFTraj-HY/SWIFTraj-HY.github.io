## Data Fields 数据字段说明（Per-track schema：一辆车一条记录）
...
| `preceding_id` | list[int] | Preceding vehicle id in the same lane / 同车道前车编号 | Use a consistent “no vehicle” code (e.g., `-1` or `0`) / 无前车时请统一缺失编码（如 `-1` 或 `0`） |
| `following_id` | list[int] | Following vehicle id in the same lane / 同车道后车编号 | Use a consistent “no vehicle” code (e.g., `-1` or `0`) / 无后车时请统一缺失编码（如 `-1` 或 `0`） |
...
| `pixel_corners` | list[list[float]] | 4 corners per frame in pixels (px): `[x1,y1,x2,y2,x3,y3,x4,y4]` / 每帧4角点像素坐标（像素，8浮点数） | Corner order is normalized: start from the top-most & left-most point, then clockwise; aligned with `frame_index` / 角点顺序已规范：从最上且最左点开始，顺时针；与 `frame_index` 对齐 |
| `ground_corners` | list[list[float]] | 4 corners per frame in ground coords (m), mapped from `pixel_corners` via affine transform / 每帧4角点本地/地面坐标（米），由 `road_config.pixel2xy_matrix` 对 `pixel_corners` 仿射映射得到 | Corner order is identical to `pixel_corners` / 角点顺序与 `pixel_corners` 完全一致 |